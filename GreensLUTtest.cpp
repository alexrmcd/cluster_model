...
std::vector<double> createLUT(double n_r, double n_rootdv){
	...
	for (int j = 1 ; j < n_r +1 ; ++j ){
		...
		for(int i = 1; i < n_rootdv + 1; ++i){
			double drootdv = rootdv_max/n_rootdv;

			Greens_lookup[j + n_r*i] = Greens(j*dr, i*rootdv_scale);
			//std::cout  << j + n_r*i  << "  " << Greens(j*dr, i*rootdv_scale)<<std::endl;
		}


		std::cout << j << "/" << n_r <<" ";
		double rt = 0.001*mpc2cm;
		double rdv = 0.015*mpc2cm;
		double Gactual = Greens(rt, rdv);
		double Gi = (int)(rt/r_scale) + (int)(rdv/rootdv_scale)*n_r ; 
		double Glut = Greens_lookup[Gi];
		std::cout <<Gi <<" r = " <<j*dr/mpc2cm*1000<<" rt = "<< rt/mpc2cm*1000 << " rdv = "<<rdv/mpc2cm*1000;
		std::cout <<" , "<<(Glut - Gactual)/Gactual	<< std::endl;
	}
	////////after algorithm
...
void runGreens(double r){ //runs through values of root_dv
	...
	makefilename << "Greens_NFW_r" << r << "kpc" <<"GLUT_Test.txt" ;
